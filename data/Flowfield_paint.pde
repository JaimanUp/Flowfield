...
int fieldResolution=15; // How large is each "cell" of the flow field
...
void draw() {
  ...
  //if(cleanWindow==true){
  // traceWindow.background(255,0);

  //  cleanWindow=false;}
  // traceWindow.background(255,200,200,0);
  // println(cleanWindow);
  traceWindow.strokeWeight(1);
  traceWindow.stroke(0, 120);
  for (Automata v : automatas) {
    PVector[] move=v.trace();
    //      println(mag(v.acceleration.x,v.acceleration.y));
